...
@Component({
  ...
  `<div class="card">
    ...
    <div class="card-footer" [hidden]="isCompleted">
        <button type="button" class="btn btn-secondary float-left" (click)="previous()" [hidden]="!hasPrevStep || !activeStep.showPrev">{{titlePrevious}}</button>
        <button type="button" class="btn btn-secondary float-right" (click)="next()" [disabled]="!activeStep.isValid" [hidden]="!hasNextStep || !activeStep.showNext">{{titleNext}}</button>
        <button type="button" class="btn btn-secondary float-right" (click)="complete()" [disabled]="!activeStep.isValid" [hidden]="hasNextStep">{{titleDone}}</button>
    </div>
  </div>`
  ,
  styles: [
...
export class WizardComponent implements AfterContentInit {
  ...
  @Input() titleNext = 'Next';
  @Input() titlePrevious = 'Previous';
  @Input() titleDone = 'Done';
